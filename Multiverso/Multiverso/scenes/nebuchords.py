...
class Nebuchord:
    def __init__(self, game, tonalidad, octave, mode):
        ...
        self.background_image = pygame.image.load(os.path.join(BASE_DIR, "assets", "images", "nebuc.png"))
        self.note_radius = 40
    ...
    def handle_events(self, events):
        """Maneja los eventos, incluyendo clic en notas."""
        for event in events:
    ...
    def on_note_clicked(self, index):
        rp.play_scale_chords(self.tonalidad, self.mode, inverso, index)
    ...
    def hls_to_platinado(self, hue, lightness=0.5):
        """hue (float): Tono en el rango [0, 1]
        lightness (float): Luminosidad (0.5-0.7 para efecto metalizado)"""
        saturation = 0.5  # Saturación moderada para mantener tonos vivos pero no pastel
        metal_intensity = 0.7  # Factor de mezcla con gris (0-1)
        gray_base = 80       # Base gris para efecto metálico
